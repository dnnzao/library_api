...
RSpec.describe BooksController, type: :request do
  ...
  let(:valid_attributes) {
    { book_name: 'The Great Gatsby', author: 'F. Scott Fitzgerald', published_date: Date.current, category_id: 1, publisher_id: 1 }
  }
    ...
    context 'when user is logged in' do
        ...
        context 'with valid parameters' do
            it 'creates a new book and returns status code 201' do
                post books_path, params: { book: valid_attributes }, headers: { Authorization: "Bearer #{user.auth_token}" }
        ...
        context 'with invalid parameters' do
            it 'does not create a new book and returns status code 422' do
                post books_path, params: { book: invalid_attributes }, headers: { Authorization: "Bearer #{user.auth_token}" }
    ...
    context 'when user is logged in' do
        ...
        context 'with valid parameters' do
            ...
            it 'updates the book and returns status code 200' do
                put book_path(book), params: { book: new_attributes }, headers: { Authorization: "Bearer #{user.auth_token}" }
        ...
        context 'with invalid parameters' do
            it 'does not update the book and returns status code 422' do
                put book_path(book), params: { book: invalid_attributes }, headers: { Authorization: "Bearer #{user.auth_token}" }
                expect(response).to have_http_status(422)
            end
        end
    end
